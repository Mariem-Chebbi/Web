...
class RendezVousController extends AbstractController
{
    ...
    public function getAllRdv(RendezVousRepository $repo): Response
    {
        ...
        $client=$this->getUser();
        $events = $repo->findBy(['id_personnel' => $client->getId()]);
        $rdvs = [];
        $data = null;
    ...
    public function deleteEvent(
        ...
        if (isset($donnees->id) && !empty($donnees->id)) {
            $mailer->sendEmail($rdv->getClient()->getEmail(),"Votre rendez-vous le ".$rdv->getDateRdv()." a été annulé","Annulation de rendez-vous");
            $code = 200;
            $entityManager = $doctrine->getManager();
            $entityManager->remove($rdv);
            $entityManager->flush();
    ...
    {
        $client=$this->getUser();
        $rdvs = $repo->orderedCurrentRdv($client->getId());
    ...
    {
        $client=$this->getUser();
        $rdvs = $repo->searchByDate($date,$client->getId());
        $results = [];
    ...
    #[Route('/rdv/history/list', name: 'list_history_ordered_rdv')]
    public function listHistoryOrderedRdv(RendezVousRepository $repo)
    {
        $client=$this->getUser();
        $id = $client->getId();
